...
In this paper, we proposed GNN-Bandit, a principled framework combining Graph Neural Networks, Batch-Constrained Reinforcement Learning, Lagrangian Optimization, and Doubly Robust Off-Policy Evaluation for safe customer retention. By utilizing LightGCN linear graph convolutions as low-pass causal spectral filters, GNN-Bandit resolves the cold-start challenge in offline policy learning, while BCQ action filtering eliminates distributional extrapolation errors. Grounding offline constraints in Lagrangian duality provides a mathematically unified foundation for real-world enterprise budget pacing. Exhaustive empirical validation across six benchmark datasets (more than 40 million logged interactions) shows statistically significant gains of \textbf{+16.08\% to +25.31\%} over the strongest baseline on all three interaction-graph OBD campaigns, consistent improvements over the historical logging policy on every benchmark, and cold-start resilience---while transparently delimiting the boundary conditions (weak or absent collaborative graphs) under which context-only and unconstrained methods remain competitive. This applicability boundary, established empirically and motivated by the low-pass spectral analysis, is itself a core deliverable: it tells practitioners precisely when graph-enhanced causal policy learning pays off. GNN-Bandit thereby establishes a rigorous and deployable blueprint for high-stakes offline decision support.
