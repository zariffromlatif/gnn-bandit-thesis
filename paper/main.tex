...
Let $\mathcal{P}_X$ and $\mathcal{Q}_X$ denote the feature distributions of treated and control populations over the graph. Let $\Phi^{(l)}(X) = \tilde{\mathbf{A}}^l X$ be the node representation after $l$ layers of normalized graph convolution. Under the family of Lipschitz continuous test functions $\mathcal{F} = \{f : \|f\|_{\mathrm{Lip}} \le 1\}$, the Integral Probability Metric (IPM) contracts exponentially:
\begin{equation}
\mathrm{IPM}_{\mathcal{F}}(\mathcal{P}_{\Phi}^{(l)}, \mathcal{Q}_{\Phi}^{(l)}) \le (1 - \lambda_2(\mathbf{L}_{\mathrm{sym}}))^l \cdot \mathrm{IPM}_{\mathcal{F}}(\mathcal{P}_X, \mathcal{Q}_X)
\end{equation}
where $\lambda_2(\mathbf{L}_{\mathrm{sym}}) > 0$ is the spectral gap (Fiedler eigenvalue) of the normalized graph Laplacian.
\end{lemma}
\begin{proof}
By the Kantorovich-Rubinstein duality theorem, $\mathrm{IPM}_{\mathcal{F}}(\mathcal{P}, \mathcal{Q}) = \mathcal{W}_1(\mathcal{P}, \mathcal{Q})$, the 1-Wasserstein distance. Symmetrically normalized graph convolution corresponds to a linear operator with operator norm bounded by the second largest singular value $\sigma_2(\tilde{\mathbf{A}}) = 1 - \lambda_2(\mathbf{L}_{\mathrm{sym}})$. For any 1-Lipschitz function $f$, $\|f \circ \tilde{\mathbf{A}}^l\|_{\mathrm{Lip}} \le \|\tilde{\mathbf{A}}\|_2^l \|f\|_{\mathrm{Lip}} \le (1 - \lambda_2)^l$. Taking the supremum over $\mathcal{F}$ yields the geometric contraction bound.
\end{proof}
...
\begin{equation}
V(\pi^*) - V(\hat{\pi}_{\mathrm{BCQ}}) \le \frac{2}{1 - \gamma} \epsilon_{\mathrm{extrap}}(\tau) + \mathcal{O}\left( \sqrt{\frac{d_{\mathcal{H}} \log(N)}{N}} \right) + (1 - \beta) \sqrt{\mathrm{PEHE}(\hat{\tau})}
\end{equation}
...
\end{theorem}
\begin{proof}
The policy suboptimality decomposes into out-of-support action truncation error $\epsilon_{\mathrm{extrap}}(\tau)$, standard sample complexity error bounded by empirical VC/Rademacher dimensions $\mathcal{O}(\sqrt{d_{\mathcal{H}} \log(N)/N})$, and CATE reward augmentation error scaled by $(1 - \beta)\sqrt{\mathrm{PEHE}(\hat{\tau})}$. When logging support is dense on the optimal action, $\epsilon_{\mathrm{extrap}}(\tau) = 0$, guaranteeing near-optimal generalization.
\end{proof}
