...
\begin{equation}
\mathbf{g}(\hat{\mathbf{a}}_0) = \nabla_{\hat{\mathbf{a}}_0} \sum_{k=1}^K p_k \tau_k = \frac{1}{T_{\mathrm{temp}}} \sum_{k=1}^K p_k (\tau_k - \mathbf{p}^\top \boldsymbol{\tau}) \tilde{\mathbf{e}}_k = \frac{1}{T_{\mathrm{temp}}} \tilde{\mathbf{E}}^\top \Big( \mathbf{p} \odot (\boldsymbol{\tau} - \mathbf{p}^\top \boldsymbol{\tau}) \Big)
\end{equation}
...
To evaluate whether generative continuous action diffusion can effectively navigate offline policy spaces and respond to test-time causal steering, we benchmark \textbf{Diff-GNN-Bandit} against discrete batch-constrained RL on KuaiRand (50 discrete actions) across five independent random seeds (0--4). Table~\ref{tab:diffusion_audit} reports Doubly Robust (DR), Self-Normalized Inverse Propensity Scoring (SNIPS), Direct Method (DM), Inverse Propensity Weighting (IPW), and per-sample GPU inference latency on an NVIDIA RTX 4090.

\begin{table}[t]
\centering
\small
\caption{Generative Score-Based Diffusion Policy performance and latency audit on KuaiRand (50 actions, Mean $\pm$ Std across 5 seeds: 0--4). Asterisks denote two-sided paired Student's $t$-test significance against Unguided Diffusion ($^{***}p < 0.001$).}
\label{tab:diffusion_audit}
\begin{tabular}{lccccc}
\toprule
\textbf{Policy Architecture} & \textbf{DR Value} & \textbf{SNIPS} & \textbf{DM} & \textbf{IPW} & \textbf{Latency / sample} \\
\midrule
...
\midrule
Tweedie Guidance Lift (\%) & \textbf{+1.99\%} & \textbf{+2.07\%} & \textbf{+1.79\%} & \textbf{+1.85\%} & +0.56 $\mu$s overhead \\
Statistical Significance ($t$-test) & $p = 1.93 \times 10^{-5}$ & $p = 7.98 \times 10^{-5}$ & $p = 1.25 \times 10^{-4}$ & $p = 1.88 \times 10^{-6}$ & Highly Significant \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{enumerate}
    \item \textbf{High-Significance Causal Steering}: Incorporating analytical Tweedie CATE guidance during reverse DDIM sampling generates a consistent and statistically decisive improvement over unguided score diffusion across all counterfactual metrics ($+1.99\%$ DR, $p = 1.93 \times 10^{-5}$; $+2.07\%$ SNIPS, $p = 7.98 \times 10^{-5}$). The closed-form gradient successfully shifts probability mass toward treatment arms exhibiting positive uplift without requiring retraining of the underlying score network.
    \item \textbf{Surpassing Historical Logging}: Both unguided ($0.4058 \pm 0.0021$) and guided ($0.4139 \pm 0.0026$) diffusion policies substantially exceed the historical logging policy baseline ($0.3995 \pm 0.0046$, up to $+3.60\%$ lift), confirming that score-based generative modeling captures high-fidelity behavior priors on graph embeddings.
    \item \textbf{Discrete Value Optimization vs. Continuous Generation}: Discrete GNN-Bandit (BCQ) achieves a higher policy return ($0.5205 \pm 0.0029$), reflecting the fact that Q-value actor-critic networks directly optimize expected return over the discrete action support, whereas unconditional score diffusion primarily models the underlying logging density before test-time steering.
    \item \textbf{Sub-3 Microsecond Real-Time SLA}: Fast 15-step DDIM sampling executes in $2.32 \ \mu\text{s}$ per query sample, while analytical Tweedie guidance adds merely $0.56 \ \mu\text{s}$ of computation, totaling $2.88 \ \mu\text{s}$ ($< 0.003 \ \text{ms}$). This satisfies strict commercial production latency SLAs ($< 35 \ \text{ms}$) by four orders of magnitude, establishing the practical viability of score-based diffusion in large-scale online recommendation serving.
\end{enumerate}
